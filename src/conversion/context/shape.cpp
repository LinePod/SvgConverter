...
void ShapeContext::path_cubic_bezier_to(double x1, double y1, double x2, double y2, double x, double y,
                                        svgpp::tag::coordinate::absolute) {
    auto ctrl1 = to_global_point(x1, y1);
    auto ctrl2 = to_global_point(x2, y2);
    auto target = to_global_point(x, y);
    std::cout << "Cubic bezier curve to " << target << " with control points " << ctrl1 << " and " << ctrl2 << '\n';
}
